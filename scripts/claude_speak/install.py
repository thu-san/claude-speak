...
def _write_dotenv(quiet: bool) -> None:
    """Write <plugin_root>/.env with CLAUDE_PLUGIN_DATA captured from the
    installer's env. SessionStart hook always has this env var set by
    Claude Code, so this fires exactly once per install and gives the
    correct value.

    Users source the file in their shell to make terminal CLIs work
    (`set -a; source .env; set +a`). Python code never reads this file —
    that would risk reading stale values when the active install changes.
    Gitignored; guarded by CLAUDE.md against commits."""
    import os
    data = os.environ.get("CLAUDE_PLUGIN_DATA")
    if not data:
        return  # nothing to persist (shouldn't happen from SessionStart)
    # config.py → claude_speak/ → scripts/ → <plugin_root>/
    plugin_root = Path(__file__).resolve().parent.parent.parent
    env_file = plugin_root / ".env"
    try:
        env_file.write_text(
            "# Auto-generated by claude-speak install. Do NOT commit.\n"
            "# Source this in your shell to run terminal CLIs:\n"
            "#   set -a; source .env; set +a\n"
            f"CLAUDE_PLUGIN_DATA={data}\n"
        )
        _say(f"wrote {env_file} (source it for terminal CLI use)", quiet,
             notify_too=False)
    except OSError as e:
        _say(f"could not write {env_file}: {e}", quiet, notify_too=False)
...
def main() -> int:
    ...
    if not args.force and MARKER.exists():
        ...
        # Always refresh .env on SessionStart even when the install itself is
        # skipped — if the user switched marketplaces (@thu-san ↔ @local), the
        # old .env could be stale and terminal CLIs would land in the wrong
        # dir after sourcing it.
        _write_dotenv(args.quiet)
